...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction: `You are a Blostem Financial Advisor. You only have access to the following Supported Banks and their Current FD Rates: ${rateString}. When a user asks about FDs, asks for recommendations, or asks which bank to choose, you MUST ONLY suggest banks from this exact list. Under no circumstances should you mention, suggest, or compare banks that are not in this list. Calculate the estimated maturity amount (using standard annual compound interest) and explain it conversationally in ${language}. If the user confirms they want to book, you must return ONLY a raw JSON object matching this schema using the ACTUAL details from the conversation: { "booking_intent": true, "bankName": "<Bank they chose>", "amount": <number requested>, "tenor": "<tenor requested>", "interestRate": <rate for that bank>, "maturityAmount": <calculated number> }. Do NOT include any conversational text, pleasantries, or markdown formatting before or after the JSON.`,
        temperature: 0.2, 
      }
    });
